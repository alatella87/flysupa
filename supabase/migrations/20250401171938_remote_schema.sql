...
create policy "Allow users to read their own licenses"
on "storage"."objects"
as permissive
for delete
to public
using (((bucket_id = 'licenses'::text) AND (auth.uid() IS NOT NULL)));


create policy "Allow users to select their own licenses"
on "storage"."objects"
as permissive
for select
to public
...
create policy "Allow users to upload licenses"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'licenses'::text) AND (auth.uid() IS NOT NULL)));
